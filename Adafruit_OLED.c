...
*/
    ...
    // Set DC pin to low
    GPIOPinWrite(GPIOA1_BASE, 0x10, 0); // was 0x2 pin 64

    // Activating OLED device
    GPIOPinWrite(GPIOA2_BASE, 0x40, 0);

    // Send the command byte using SPI
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, &dum);

    // Deactivating OLED device
...
*/
    ...
    // Set DC pin to indicate data
    GPIOPinWrite(GPIOA1_BASE, 0x10, 0x10); // was 0x2 pin 64

    // Select the OLED device (assert OLEDCS)
    GPIOPinWrite(GPIOA2_BASE, 0x40, 0);

    // Send the data byte using SPI
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, &data);

    // Deselect the OLED device (deassert OLEDCS)
    GPIOPinWrite(GPIOA2_BASE, 0x40, 0x40);
